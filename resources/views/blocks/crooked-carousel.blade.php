<section class="crooked-carousel-block bg-blue py-14 lg:py-24">
    <div class="container">
        ...
        @if (!empty($subtitle))
            <p class="callout-small text-dark-blue mb-16 text-center">{!! wp_kses_post($subtitle) !!}</p>
        @endif
            ...
            <div class="swiper-wrapper pb-8">
                @foreach ($items as $item)
                    <div class="swiper-slide">
                        <div
                            class="crooked-card flex aspect-square flex-col justify-start rounded-sm bg-white p-4 lg:p-10">
                            @if (!empty($item['stat']))
                                <p class="stat text-dark-blue mb-4">{!! wp_kses_post($item['stat']) !!}</p>
                            @endif

                            @if (!empty($item['description']))
                                <p class="text-xlarge-union text-dark-blue">{!! wp_kses_post($item['description']) !!}</p>
                            @endif
                        </div>
                    </div>
                @endforeach
