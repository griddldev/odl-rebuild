        ...
        <div class="blog-section-featured relative flex flex-col justify-between gap-10 md:flex-row">
            @if (!empty($featuredPost))
                <div class="flex w-full flex-col justify-between md:w-[50%]">
                    <h2 class="heading-2 mb-10 md:mb-0">{{ $title }}</h2>

                    @if (!empty($featuredPost['image']))
                        <div class="relatve">
                            <img src="{{ Illuminate\Support\Facades\Vite::asset('resources/images/icons/star-red.svg') }}"
                                alt=""
                                class="absolute right-[-60px] top-[53px] w-full max-w-[120px] object-cover md:hidden"
                                aria-hidden="true" />
                ...
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
                    @foreach ($latestPosts as $index => $post)
                        ...
                        <div class="blog-section-card bg-{{ $color }} block p-6"
                            data-color="{{ $color }}">
                            <a href="{{ esc_url($post['permalink']) }}" class="blog-section-card-image mb-6 block">
                                @if (!empty($post['image']))
                                    <img src="{{ esc_url($post['image']) }}"
                                        alt="{{ esc_attr($post['imageAlt'] ?: $post['title']) }}"
                                        class="aspect-[4/3] w-full object-cover" loading="lazy" />
                                @endif
                            </a>

                            @if (!empty($post['categories']))
                                <ul class="mb-6 flex flex-wrap gap-2 pl-0">
                                    @foreach ($post['categories'] as $category)
                                        <li
                                            class="blog-section-card-category text-small text-dark-blue bg-off-white list-none px-2 py-1 font-medium">
                                            {{ esc_html($category) }}
                                        </li>
                                    @endforeach
                                </ul>
